...
- The active locator is now visible in the dashboard header itself (between the clock and the version line), not just in the footer note: shows the configured grid square, "Frankfurt am Main (default)" when unset, or a highlighted "⚠ Xkm from coverage" when it's outside the far-from-coverage threshold. Each hero tile's subtitle also now shows its distance from that locator ("299km from you"), so it's visible right next to the reading it qualifies, not just in one summary line.
